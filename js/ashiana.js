...
document.addEventListener("DOMContentLoaded", function () {
  ...
  brochureForm.addEventListener("submit", async function (e) {
    ...
    const leadData = {
      ...
      project: formData.get("project") || "Ashiana"
    };
    ...
    emailjs.send(
      ...
      "template_hm3z1bq",
      {
        ...
        page_name: leadData.project,     // Ashiana
        page_url: window.location.href,
        message: leadData.source
      }
    )
    .then(() => {
      console.log("✅ Ashiana Email sent");
    })
    .catch((error) => {
      console.error("❌ Ashiana Email failed:", error);
    });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /* ===== PAGE INFO ===== */
  const pageName = document.title || "Ashiana Aaroham";
  ...
  contactForm.addEventListener("submit", async function (e) {
    ...
    try {
      await window.supabase.from("leads").insert([{
        ...
        phone,
        email,
        page_name: document.title || "Ashiana Aaroham",
        page_url: window.location.href
      }]);
