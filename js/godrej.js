...
document.addEventListener("DOMContentLoaded", function () {
  ...
  brochureForm.addEventListener("submit", async function (e) {
    ...
    const leadData = {
      ...
      project: formData.get("project") || "Godrej"
    };
    ...
    emailjs.send(
      ...
      "template_hm3z1bq",
      {
        ...
        page_name: leadData.project, // Godrej
        page_url: window.location.href,
        message: leadData.source
      }
    )
    .then(() => {
      console.log("✅ Godrej Email sent");
    })
    .catch((error) => {
      console.error("❌ Godrej Email failed:", error);
    });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const pageName = document.title || "Godrej Miraya – Sector 43";
  ...
  contactForm.addEventListener("submit", async function (e) {
    ...
    try {
      await window.supabase.from("leads").insert([{
        ...
        phone,
        email,
        page_name: document.title || "Godrej Miraya – Sector 43",
        page_url: window.location.href
      }]);
      ...
      await emailjs.send(
        "service_kabl40s",
        "template_hm3z1bq",
        { name, phone, email, page_name: document.title || "Godrej Miraya – Sector 43", page_url: window.location.href }
      );
