...
document.addEventListener("DOMContentLoaded", function () {
  ...
  brochureForm.addEventListener("submit", async function (e) {
    ...
    const leadData = {
      ...
      project: formData.get("project") || "SOBHA"
    };
    ...
    emailjs.send(
      ...
      "template_hm3z1bq",
      {
        ...
        page_name: leadData.project,      // SOBHA
        page_url: window.location.href,
        message: leadData.source
      }
    )
    .then(() => {
      console.log("✅ SOBHA Email sent");
    })
    .catch((error) => {
      console.error("❌ SOBHA Email failed:", error);
    });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const popupForm = document.getElementById("apexLeadForm1");
  ...
  /* ===== PAGE INFO ===== */
  const pageName = document.title || "Sobha Aranya Sector 80";
  ...
  contactForm.addEventListener("submit", async function (e) {
    ...
    try {
      await window.supabase.from("leads").insert([{
        ...
        phone,
        email,
        page_name: document.title || "Sobha Aranya Sector 80",
        page_url: window.location.href
      }]);
